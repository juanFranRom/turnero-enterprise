...
import { PrismaClient, ResourceKind } from '@prisma/client';
...
describe('Resources CRUD (e2e)', () => {
  ...
  it('creates + lists + gets + updates + soft deletes', async () => {
    ...
    const created = await axios.post(
      ...
      { locationId: LOCATION_ID, name, kind: ResourceKind.STAFF },
    ...
    const list = await axios.get(
      `${baseURL}/api/resources?locationId=${encodeURIComponent(LOCATION_ID)}`,
      { headers: headers(token) },
    );
    ...
    const upd = await axios.patch(
      ...
      { name: `${name} Updated`, kind: ResourceKind.ROOM },
      { headers: headers(token) },
    );
  ...
  it('validates locationId belongs to tenant (404 LOCATION_NOT_FOUND)', async () => {
    ...
    await expect(
      axios.post(
        `${baseURL}/api/resources`,
        { locationId: OTHER_LOCATION_ID, name, kind: ResourceKind.STAFF },
        { headers: headers(token) },
      ),
  ...
  it('tenant isolation: cannot read resource from another tenant (401 Invalid tenant)', async () => {
  ...
  it('enforces unique name per (tenantId, locationId, name) (409 RESOURCE_NAME_TAKEN)', async () => {
    ...
    await axios.delete(`${baseURL}/api/resources/${r1.data.id}`, {
      headers: headers(token),
    });
  });
});