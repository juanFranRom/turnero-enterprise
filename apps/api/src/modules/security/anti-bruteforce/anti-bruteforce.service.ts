...
export class AntiBruteForceService {
  ...
  private prefix() {
    return `turnero:${process.env.NODE_ENV ?? 'dev'}`;
  }
  ...
  private failIpKey(tenantId: string, ip: string) {
    return `${this.prefix()}:ab:failip:${tenantId}:${ip}`;
  }

  private lockIpKey(tenantId: string, ip: string) {
    return `${this.prefix()}:ab:lockip:${tenantId}:${ip}`;
  }

  private failKey(tenantId: string, email: string, ip: string) {
    return `${this.prefix()}:ab:fail:${tenantId}:${this.hash(email)}:${ip}`;
  }

  private lockKey(tenantId: string, email: string, ip: string) {
    return `${this.prefix()}:ab:lock:${tenantId}:${this.hash(email)}:${ip}`;
  }

  private computeLockMs(attempts: number, steps: { minAttempts: number; lockMs: number }[]) {
    let lockMs = 0;
  ...
  async checkOrThrow(tenantId: string, email: string, ip: string) {
    const emailLockTtl = await this.redis.pttl(
      this.lockKey(tenantId, email, ip),
    );

    const ipLockTtl = await this.redis.pttl(
    ...
    if (ttl && ttl > 0) {
      throw new HttpException(
        {
          message: `Too many failed attempts. Retry in ${Math.ceil(ttl / 1000)}s`,
  ...
  async onFailure(tenantId: string, email: string, ip: string) {
    ...
    if (emailAttempts === 1) {
      await this.redis.pexpire(emailFailKey, ANTI_BRUTE_FORCE_CONFIG.FAILURE_WINDOW_MS);
    }
    if (ipAttempts === 1) {
      await this.redis.pexpire(ipFailKey, ANTI_BRUTE_FORCE_CONFIG.FAILURE_WINDOW_MS);
    }

    const lockEmailMs = this.computeLockMs(emailAttempts, ANTI_BRUTE_FORCE_CONFIG.LOCK_STEPS_EMAIL);
    const lockIpMs = this.computeLockMs(ipAttempts, ANTI_BRUTE_FORCE_CONFIG.LOCK_STEPS_IP);

    if (lockEmailMs > 0) {
      await this.redis.set(this.lockKey(tenantId, email, ip), '1', 'PX', lockEmailMs);
    }
    if (lockIpMs > 0) {
      await this.redis.set(this.lockIpKey(tenantId, ip), '1', 'PX', lockIpMs);
    }
  }
