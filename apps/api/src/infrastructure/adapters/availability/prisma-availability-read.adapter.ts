...
export class PrismaAvailabilityReadAdapter implements AvailabilityReadPort {
    ...
    async assertResourceServiceAllowed({ tenantId, resourceId, serviceId }: any) {
        ...
        if (!link) throw new ForbiddenException({ code: 'SERVICE_NOT_ALLOWED_FOR_RESOURCE' });
    }
    ...
    async getBusy({ tenantId, locationId, resourceId, from, to, excludeAppointmentId }: any) {
